...
impl OnboardingPromptBlock {
    ...
    fn render_text(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const LINE_ONE: &str = "Next, let’s set up your prompt. Cortex has a custom prompt builder or you can select PS1 to honor your pre-existing prompt configuration.";
        const LINE_TWO: &str =
            "Cortex works with many custom prompts like oh-my-zsh, Starship, Powerlevel10K. ";
